...
inline void block_reduce_v1dmin(const uint ti, const uint nt, __local vec1 *tmp) {
	uint j;
	for(j = nt/2; j > 0; j >>= 1) {
	...
	__global vec1 *t2)
{
	...
	const uint nx = nbm->nx;
	const vec1 dtn = nbm->dtn;
	if (gti < nx) {
		t1[gti] = 0.0f;
		t2[gti] = dtn;
	}
...
// 		}
		if (gti != j) {
			...
			rp2 = 1.0f/(len23(dx) + eps);
			at = xj.w*rp2*sqrt(rp2);
			out.lo += at*dx;
			out.hi += at*(dv - dx*(vec1)(3.0f*dot(dx,dv)*rp2));
		}
	}
	return -out;
}
	...
	__global vec4 *v1)
{
	...
	if (gid < nbm->nu) {
		...
		if (dtt < dti && dtt > 2.0f*FLT_EPSILON)
			dtt = 0.5f*dti;
		else if (dtt > 2.0f*dti)
			dtt = 2.0f*dti;
		else
			dtt = dti;
		t1[gti] = nbm->dtn;
// 		t2[gti] = tt + dtt;
		t2[gti] = nbm->dtn + pown(2.0f, floor(log2(dtt)));
	}
}
